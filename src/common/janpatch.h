...
typedef struct JANPATCH_STREAM_s {
  uint8_t* src;
  uint32_t position;
  void*    context;
} JANPATCH_STREAM_t;

void janpatch_stream_init(JANPATCH_STREAM_t* stream, uint8_t* baseAddress, void* ctx) {
  stream->src      = baseAddress;
  stream->context  = ctx;
  stream->position = 0;
}

#define JANPATCH_STREAM JANPATCH_STREAM_t
...
typedef struct {
  ...
  // the combination of the size of both the source + patch files (that's the max. the target file
  // can be)
  long max_file_size;
} janpatch_ctx;
 ...
 */
static size_t jp_fwrite(janpatch_ctx* ctx, const void* ptr, size_t size, size_t count,
                        janpatch_buffer* buffer) {
  ctx->fseek(buffer->stream, buffer->position, SEEK_SET);
...
static void process_mod(janpatch_ctx* ctx, janpatch_buffer* source, janpatch_buffer* patch,
  ...
  while (1) {
    ...
    if (m == -1) {
      // End of file stream... rewind 1 character and return, this will yield back to janpatch main
      // function, which will exit
    ...
    else if (m >= 0xA2 && m <= 0xA6) {  // character after this is an operator? Then roll back two
                                        // characters and exit
    ...
    if (c == JANPATCH_OPERATION_ESC) {
      switch ((c = jp_getc(ctx, &ctx->patch_buffer))) {
        ...
        case JANPATCH_OPERATION_INS: {
          JANPATCH_DEBUG("INS: ");
          // INS inserts the sequence in the new JANPATCH_STREAM, but does not up the position of
          // the source JANPATCH_STREAM
          // so just write everything (until the next ESC sequence) to the target JANPATCH_STREAM
        ...
        case JANPATCH_OPERATION_BKT: {
          ...
          if (length == -1) {
            ...
            JANPATCH_ERROR("Positions are, source=%ld patch=%ld new=%ld\n",
                           ctx->source_buffer.position, ctx.patch_buffer.position, ctx->.position);
            return 1;
          }
      ...
      JANPATCH_ERROR("Positions are, source=%ld patch=%ld new=%ld\n", ctx->source_buffer.position,
                     ctx->patch_buffer.position, ctx->target_buffer.position);
